...
function sleep(minutes){
      return new Promise(resolve=>setTimeout(resolve,minutes));
    }
    async function run(){
      while(1){
        var time = new Date();
        ...
        console.log(string);
        h1.innerHTML = string;
        if(time.getMinutes()>=0 && time.getMinutes()<=15 && time.getHours()==0 && time.getMonth()==1  && time.getDate()==12){
          Run();
